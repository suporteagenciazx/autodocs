document.addEventListener('DOMContentLoaded', () => {
  ...
  const simpleMap = [
    ...
    { inputId: 'i-trust-cnpj', targetId: 'trust-cnpj', original: '[TRUST-CNPJ]' },
  ];
  ...
  function renderPlChart() {
    ...
    svg.innerHTML = '';
    const parts = [
      { label: 'Capital', v: Math.max(0, dmpl.final.cs), color: '#59111d' },
      { label: 'Res. Legal', v: Math.max(0, dmpl.final.rl), color: '#2d6a4f' },
      { label: 'Fundos', v: Math.max(0, dmpl.final.fe), color: '#8b3d4a' },
  ...
  function renderEvolChart() {
    ...
    xs.forEach((x, i) => {
      svg.appendChild(svgNode('circle', { cx: x, cy: ys[i], r: 4.5, fill: '#59111d' }));
      svg.appendChild(
        svgNode(
          'text',
          {
