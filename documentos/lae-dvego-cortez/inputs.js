document.addEventListener('DOMContentLoaded', () => {
  ...
  const simpleMap = [
    ...
    { inputId: 'i-cortez-cnpj', targetId: 'cortez-cnpj', original: '[CORTEZ-CNPJ]' },
  ];
  ...
  function renderPlChart() {
    ...
    svg.innerHTML = '';
    const parts = [
      { label: 'Capital', v: Math.max(0, dmpl.final.cs), color: '#0020EF' },
      { label: 'Res. Legal', v: Math.max(0, dmpl.final.rl), color: '#2d6a4f' },
      { label: 'Fundos', v: Math.max(0, dmpl.final.fe), color: '#3355F2' },
  ...
  function renderEvolChart() {
    ...
    xs.forEach((x, i) => {
      svg.appendChild(svgNode('circle', { cx: x, cy: ys[i], r: 4.5, fill: '#0020EF' }));
      svg.appendChild(
        svgNode(
          'text',
          {
