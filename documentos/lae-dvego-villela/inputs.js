document.addEventListener('DOMContentLoaded', () => {
  ...
  const simpleMap = [
    ...
    { inputId: 'i-villela-cnpj', targetId: 'villela-cnpj', original: '[VILLELA-CNPJ]' },
  ];
  ...
  function renderPlChart() {
    ...
    svg.innerHTML = '';
    const parts = [
      { label: 'Capital', v: Math.max(0, dmpl.final.cs), color: '#14324A' },
      { label: 'Res. Legal', v: Math.max(0, dmpl.final.rl), color: '#2d6a4f' },
      { label: 'Fundos', v: Math.max(0, dmpl.final.fe), color: '#3d5a73' },
  ...
  function renderEvolChart() {
    ...
    xs.forEach((x, i) => {
      svg.appendChild(svgNode('circle', { cx: x, cy: ys[i], r: 4.5, fill: '#14324A' }));
      svg.appendChild(
        svgNode(
          'text',
          {
