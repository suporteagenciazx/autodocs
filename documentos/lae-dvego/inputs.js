document.addEventListener('DOMContentLoaded', () => {
  ...
  const simpleMap = [
    ...
    { inputId: 'i-aguia-cnpj', targetId: 'aguia-cnpj', original: '[AGUIA-CNPJ]' },
  ];
  ...
  function renderPlChart() {
    ...
    svg.innerHTML = '';
    const parts = [
      { label: 'Capital', v: Math.max(0, dmpl.final.cs), color: '#2e2e2e' },
      { label: 'Res. Legal', v: Math.max(0, dmpl.final.rl), color: '#5a5a5a' },
      { label: 'Fundos', v: Math.max(0, dmpl.final.fe), color: '#8a8a8a' },
      { label: 'Lucros', v: Math.max(0, dmpl.final.la), color: '#b0b0b0' },
    ];
  ...
  function renderEvolChart() {
    ...
    xs.forEach((x, i) => {
      svg.appendChild(svgNode('circle', { cx: x, cy: ys[i], r: 4.5, fill: '#2e2e2e' }));
      svg.appendChild(
        svgNode(
          'text',
          {
