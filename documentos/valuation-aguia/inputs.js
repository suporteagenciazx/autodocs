document.addEventListener('DOMContentLoaded', () => {
    ...
    const configurations = [
        ...
        { inputId: 'i-aguia-cnpj', targetId: 'aguia-cnpj', originalText: '[AGUIA-CNPJ]' },
    ];
        ...
        for (let i = 0; i <= 4; i++) {
            ...
            if (grid) {
                grid.appendChild(svgNode('line', {
                    ...
                    stroke: '#e6e6e6', 'stroke-width': 1, 'stroke-dasharray': '4 5',
                }));
            }
            if (yLabels && max > 0) {
        ...
        CHART.xs.forEach((x, i) => {
            if (points) {
                points.appendChild(svgNode('circle', {
                    cx: x, cy: ys[i], r: 5, fill: '#4a4a4a', stroke: '#fff', 'stroke-width': 3,
                }));
            }
            if (valueLabels) {
